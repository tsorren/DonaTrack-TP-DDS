...
1. Resolver el duplicado `EventoDonacionSegmentadaDTO` (donaciones-service) vs `EventoDonacionSegmentadaV1` — confirmado con grep que en `donaciones-service` el `DTO` viejo no tiene ningún uso en código (solo aparece su propia declaración). **Ojo:** el Safe Delete de IntelliJ va a advertir que el nombre de la clase aparece mencionado en esta misma bitácora (texto plano en el `.md`) — es solo un hit en comentarios/documentación, no un uso real de código, así que no bloquea el borrado (dar "Delete Anyway"). Aparte, existe una clase con el mismo nombre `EventoDonacionSegmentadaDTO` en `incentivos-service` (su propia copia del contrato, por diseño — ver nota de `sonar.cpd.exclusions` en el `pom.xml` raíz) — esa es un archivo totalmente distinto y no se toca.
2. Retirar `NotificacionesFeignClient`/`IncentivosFeignClient`/`LogisticaFeignClient` (y sus wrappers `LogisticaAsyncService`/`ILogisticaAsyncService`) cuando se decida la limpieza global (conservando los endpoints de los controllers, decisión ya tomada en la reunión). `LogisticaFeignClient` se sumó a esta lista recién ahora, tras cablear `donacion.asignada`.
3. Limpieza aparte (no bloqueante): refrescar `matriz-productor-consumidor.md` completo, tiene contenido pre-reunión del 15/9 que ya no coincide con `catalogo-mensajes.md`.
4. Agregar DLX al `LogisticaEventListener` existente.
5. Más adelante (no ahora): persistencia real (JPA/Postgres) de los 7 agregados que ahora hacen falta (los 6 originales + `Donacion`, por `donacion.segmentada`), y swap del Outbox en memoria al real.
