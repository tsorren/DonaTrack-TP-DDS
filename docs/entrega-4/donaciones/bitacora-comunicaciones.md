...
1. Arreglar `scripts/validate-contracts.js` — el único test que falla (`evento_donacion_asignada_v1_valido`, 136/137) todavía usa el campo viejo `personaDonanteId` en el payload de prueba en vez de `donanteId`+`personaId` (el schema real ya se partió). Es un fix de una línea en el test, no del schema. Nico lo había marcado como paso 0 antes de seguir; quedó sin hacer, a la espera de que lo revise él.
2. Resolver el duplicado `EventoDonacionSegmentadaDTO` (donaciones-service) vs `EventoDonacionSegmentadaV1` — confirmado con grep que en `donaciones-service` el `DTO` viejo no tiene ningún uso en código (solo aparece su propia declaración). **Ojo:** el Safe Delete de IntelliJ va a advertir que el nombre de la clase aparece mencionado en esta misma bitácora (texto plano en el `.md`) — es solo un hit en comentarios/documentación, no un uso real de código, así que no bloquea el borrado (dar "Delete Anyway"). Aparte, existe una clase con el mismo nombre `EventoDonacionSegmentadaDTO` en `incentivos-service` (su propia copia del contrato, por diseño — ver nota de `sonar.cpd.exclusions` en el `pom.xml` raíz) — esa es un archivo totalmente distinto y no se toca.
3. Retirar `NotificacionesFeignClient`/`IncentivosFeignClient`/`LogisticaFeignClient` (y sus wrappers `LogisticaAsyncService`/`ILogisticaAsyncService`) cuando se decida la limpieza global (conservando los endpoints de los controllers, decisión ya tomada en la reunión). `LogisticaFeignClient` se sumó a esta lista recién ahora, tras cablear `donacion.asignada`.
