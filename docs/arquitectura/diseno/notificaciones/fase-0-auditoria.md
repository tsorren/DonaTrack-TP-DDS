...
15. **[MEDIO] `Notificacion` no usa la base genérica de domain events que ya existe en `common-lib`.** `common-lib` ya tiene `AgregadoConEventos<E extends EventoDeDominio>` (acumulación + snapshot + limpieza) y `EventoDeDominio` (id/timestamp autogenerados), construidos por `incentivos-service`. El mecanismo de domain events que este servicio necesita (RF-02) puede y debería apoyarse en esa base en vez de reimplementarla local. Ver Fase 0.5 y RF-02.
16. **[BAJO] Divergencia DC↔código en `Notificacion`.** El DC (CSV) dibuja `persona: Persona` como atributo y `notificar(NotificacionSender)`/`ordenarMedios(): List<MedioDeContacto>` sin parámetros. El código real no tiene ningún atributo `Persona` en `Notificacion` — solo `personaId: UUID` — y `notificar(Persona, NotificacionSender)`/`ordenarMedios(Persona)` reciben la `Persona` completa como parámetro en el momento del envío. Es una decisión de diseño ya tomada (evita que el agregado `Notificacion` mantenga una referencia directa a otro agregado, `Persona`), coherente con el eje "Límites de Agregados" de la Oleada 10 — el DC quedó desactualizado respecto de esa decisión, no el código. Ver §3.
...
| `Notificacion` | ✅ Sí | `models/entities/notificaciones/Notificacion.java` | Código agrega `actualizarEstado()` (público, no está en el DC) que es quien alimenta `historialEstado`; `ordenarMedios()` es público en código, el DC lo dibuja `-` (privado). **Divergencia real (hallazgo #16):** el DC dibuja `persona: Persona` como atributo; el código no tiene ningún atributo `Persona` — solo `personaId: UUID` — y `notificar`/`ordenarMedios` reciben la `Persona` completa como parámetro en el momento del envío, no como campo. Decisión de diseño ya tomada (límite de agregado), el DC no la refleja. |
...
**Trazabilidad / validación transversal:** `GlobalExceptionHandler` de `common-lib` ya cubre `RecursoNoEncontradoException`, `ValidationException`, `BusinessStateException`, `InfrastructureException`, `FeignException`, `IllegalArgumentException`, `Exception` genérica, y también `MethodArgumentNotValidException`, `HandlerMethodValidationException`, `ConstraintViolationException`, `MissingRequestHeaderException`, `MissingServletRequestParameterException`, `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException` y `DateTimeParseException` (confirmado leyendo el código fuente). Un UUID o una fecha malformada en un path variable de cualquiera de los 2 controllers ya cae en 400, no en 500. `ErrorResponse` además soporta una lista de `FieldErrorDTO` (campo, mensaje, valor rechazado) para errores de validación por campo.

`TraceResponseHeaderFilter`/`FeignTraceRequestInterceptor` (`grupo5.common.logging`) ya existen en `common-lib`, y `LoggingAutoConfiguration` los registra como `@Bean` `@ConditionalOnMissingBean` — se activan automáticamente en cualquier servicio que dependa de `common-lib` (vía `CommonLibAutoConfiguration`), sin necesidad de configuración adicional ni de `scanBasePackages` explícito. `notificaciones-service` ya emite el header `X-Trace-Id` en sus respuestas HTTP sin haber tenido que construir nada — confirmado en la Fase 0.5.
...
Dependencias: common-lib tiene AgregadoConEventos<E extends EventoDeDominio>/EventoDeDominio (construidos
  por incentivos-service — ver Fase 0.5). La implementación de este RF debería extender esa base en vez de
  reimplementar domainEvents/getDomainEvents()/clearDomainEvents() local (hallazgo #15 del resumen ejecutivo).
...
No es el primer servicio en refactorizarse: `donaciones-service` y `logistica-service` ya tienen varias oleadas propias, e `incentivos-service` (Oleada 14, "Centralización de Domain Events, Polimorfismo en Misiones, Tell Don't Ask y Normalización de Fixtures") ya integró trabajo a `common-lib` a través de la rama de integración `E4_refactor`. `common-lib` tiene abstracciones de Domain Events y de trazabilidad reales, construidas y con tests propios (`AgregadoConEventosTest`, `GlobalExceptionHandlerTest`, `TraceResponseHeaderFilterTest`, `ErrorResponseTest`).
...
| `GlobalExceptionHandler` | ✅ Sí (indirectamente, vía `@RestControllerAdvice` global) | **Reutilizar tal cual — ya está completo.** Cubre `MethodArgumentNotValidException`, `HandlerMethodValidationException`, `ConstraintViolationException`, `MissingRequestHeaderException`, `MissingServletRequestParameterException`, `HttpMessageNotReadableException`, `MethodArgumentTypeMismatchException`, `DateTimeParseException` y `FeignException`, además de las excepciones propias de dominio. Un tipo/fecha malformada en un path variable ya cae en 400, no en 500 — no hace falta ninguna acción de `notificaciones-service` en `common-lib` para esto. |
...
| **`AgregadoConEventos<E extends EventoDeDominio>`, `EventoDeDominio`** (`grupo5.common.events`) | ⚠️ **Existe, pero `Notificacion` NO lo usa.** `AgregadoConEventos` da `registrarEvento(E)`/`getDomainEvents()` (con `List.copyOf`)/`clearDomainEvents()` implementando `AggregateRoot`; `EventoDeDominio` da `id`/`timestamp` autogenerados. Es exactamente el mecanismo que RF-02 necesita, construido por `incentivos-service`. | **Migrar `Notificacion` para extender `AgregadoConEventos<NotificacionDomainEvent>`** en vez de reimplementar `domainEvents`/`getDomainEvents()`/`clearDomainEvents()` localmente. Ver hallazgo #15 del resumen ejecutivo. |
| **`TraceResponseHeaderFilter`, `FeignTraceRequestInterceptor`** (`grupo5.common.logging`) | ✅ Sí, automáticamente. `LoggingAutoConfiguration` los registra como `@Bean` (`@ConditionalOnMissingBean`), y `CommonLibAutoConfiguration` importa `LoggingAutoConfiguration` — cualquier servicio que dependa de `common-lib` los recibe sin configuración adicional. | **Reutilizar tal cual — ya está activo.** `notificaciones-service` ya devuelve `X-Trace-Id` en sus respuestas HTTP. `FeignTraceRequestInterceptor` no tiene efecto acá porque el servicio no hace llamadas salientes vía Feign (no aplica, no es una brecha). |
| `FieldErrorDTO`, `ErrorResponse` (con lista de `errors: List<FieldErrorDTO>`) | ⚠️ Disponible, todavía sin consumidor en notificaciones (no hay Bean Validation en los DTOs propios — RF-09/Oleada 9). | Una vez que Oleada 9 agregue `@NotNull`/`@NotBlank` a los DTOs de notificaciones, los errores de validación van a llegar automáticamente con este formato (vía `MethodArgumentNotValidException`, ya manejado) — no hace falta construir nada adicional en `common-lib` para esto. |
| **`ShedLock`/coordinación de schedulers** | No aplica — este servicio no tiene schedulers. | Sin acción. |

**Conclusión de Fase 0.5:** `common-lib` ya tiene una base madura de excepciones, logging, trazabilidad HTTP/Feign y Domain Events genéricos. El único punto de fricción real para `notificaciones-service` es que su implementación de Domain Events (RF-02) no usa `AgregadoConEventos<E>` — hay que evaluar si conviene migrarla (no se resuelve en esta auditoría, que no modifica código). Notificaciones no es el primer servicio en construir esta base, así que le corresponde heredarla en vez de reimplementarla.
