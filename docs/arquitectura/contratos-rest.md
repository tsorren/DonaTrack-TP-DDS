...
| Método | Endpoint | Descripción | Request DTO / Schema | Códigos HTTP |
|---|---|---|---|:---:|
| `POST` | `/api/notificaciones/eventos` | **[Canónico]** Despacho asíncrono de alertas multicanal vía payload polimórfico REST con deduplicación por Transactional Inbox (`eventId`) | [`evento-notificable.schema.json`](./contratos/schemas/evento-notificable.schema.json) | `202`, `400` |
| `POST` | `/notificaciones` | **[DEPRECATED]** Alias legacy de despacho de eventos (genera fallback `UUID` si falta `eventId`, ver [DTI-13](../adr/DEUDA_TECNICA.md#dti-13-migracion-de-clientes-consumidores-de-la-api-rest-deprecada-de-notificaciones-a-ruta-canonica-y-amqp)) | [`evento-notificable.schema.json`](./contratos/schemas/evento-notificable.schema.json) | `202`, `400` |
...
| `GET` | `/notificaciones/persona/{personaId}` | **[DEPRECATED]** Alias legacy de consulta de historial por persona | — | `200` |
...
> - **D7 (Ruta canónica `/api/notificaciones/eventos`):** El endpoint `/notificaciones` se conserva exclusivamente por retrocompatibilidad con los clientes Feign actuales de `donaciones-service` e `incentivos-service`. La migración definitiva de estos clientes está catalogada formalmente en la deuda técnica [DTI-13](../adr/DEUDA_TECNICA.md#dti-13-migracion-de-clientes-consumidores-de-la-api-rest-deprecada-de-notificaciones-a-ruta-canonica-y-amqp).
> - **D8 (Ingesta Primaria AMQP):** El canal primario de ingesta para eventos de dominio es la cola RabbitMQ `cola.eventos.notificaciones` en el exchange `notificaciones.exchange` (ver [`eventos-amqp.md`](./eventos-amqp.md)).
