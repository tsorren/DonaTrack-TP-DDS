...
|---|---|---|---|---|:---:|---|
| **AP-01** | `integration-tests`<br>`ContractIT.java` | **Green Smoke Contract**<br>*(Fowler / Khorikov)* | `integration-tests/src/test/java/grupo5/tests/contract/ContractIT.java:16-40`<br>Solo verifica `body("paths.\"/api/...\".post", notNullValue())`. No valida esquemas, tipos de datos, enums, obligatoriedad ni status codes devueltos. | **Falsa sensación de seguridad crítica:** Modificaciones incompatibles en request/response DTOs pasan desapercibidas en los tests de contrato de Java. Pasa siempre mientras el path exista. | 🔴 Crítico | 🟢 **RESUELTO** (commit `e2f1d67c` y refuerzo Surefire/CI)<br>Validación bidireccional en runtime con `Atlassian Swagger Request Validator` (`OpenApiValidationFilter`), auditoría de schemas y nulabilidad en CI (`validate-contracts.js`), y suites unitarias de drift (`OpenApiContractDriftTest`) en los microservicios. |
...
1. **Fase 1 (Runtime Failsafe):** En el commit `e2f1d67c`, `ContractIT.java` fue refactorizado incorporando la dependencia `com.atlassian.oai:swagger-request-validator-restassured:2.44.1`. Las peticiones y respuestas se validan bidireccionalmente contra el endpoint OpenAPI (`/v3/api-docs`) de cada microservicio mediante `OpenApiValidationFilter`, incorporando un test adversarial (`testAdversarialBreakingChangeContractValidation`) contra roturas de contrato.
2. **Fase 2 (Validación Estricta de Nulabilidad y Tipos en Tiempo Unitario y CI):** Para erradicar divergencias como las de Jackson serializando `null` en propiedades opcionales OpenAPI 3.0 (§1.2), se integró la auditoría semántica de contratos en `scripts/validate-contracts.js` ejecutada por `.github/workflows/agent-governance.yml`, junto con la suite `OpenApiContractDriftTest` y verificación en slices `@WebMvcTest` con `swagger-request-validator-mockmvc` / `swagger-request-validator-core` ejecutada en Surefire (<100ms) sin dependencias de Docker ni `@SpringBootTest`.
