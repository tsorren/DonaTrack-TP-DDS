...
`[OBSERVED]` En 17 clases de prueba de controladores individuales (ej. `donaciones-service`, `logistica-service`, `incentivos-service` y `PersonasControllerTest` en `notificaciones-service`), se inicializa MockMvc con:
```java
