...
|---|---|---|---|---|---|
| **Unitarias de Dominio** | `common-lib`<br>`donaciones`<br>`logistica`<br>`incentivos`<br>`notificaciones` | 174 archivos `*Test.java`<br>(182 clases de test) | 1118 métodos `@Test`<br>(~2020 ejecuciones Maven) | JUnit 5 + Mockito<br>(en memoria, sin Spring context) | 🟢 **Excelente:** Rápida (~15s), determinística, alta protección de reglas de negocio. |
...
*\*Nota metodológica sobre `standaloneSetup`: El conteo de 17 clases corresponde estrictamente a pruebas unitarias de controladores REST individuales. En el monorepo existen 19 archivos que contienen `standaloneSetup`, ya que se excluyen de esta métrica dos pruebas de validación transversal de infraestructura HTTP: `HandlersWebMvcValidationTest` (en `incentivos-service`) y `ValidacionHttpTest` (en `logistica-service`).*
