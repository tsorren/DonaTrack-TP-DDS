...
class DonacionIndependienteNotificacionesListenerFeignFallosTest {
  ...
  @Test
  void onEventoRutaIniciada_cuandoFeignFalla_noLanzaExcepcion() {
    ...
    assertDoesNotThrow(() -> listener.onEventoRutaIniciada(event));
  }

  @Test
  void onEventoDonacionRecibida_cuandoIncentivosFeignFalla_noLanzaExcepcion() {
    ...
    assertDoesNotThrow(() -> listener.onEventoDonacionRecibida(event));
  }

  @Test
  void onEventoDonacionRecibida_cuandoNotificacionesFeignFalla_noLanzaExcepcion() {
    EventoDonacionRecibida event =
        new EventoDonacionRecibida(
            donacionIndependienteId, donacionOriginalId, necesidadId, "ABC-123");
    doThrow(new RuntimeException("notificaciones-service no disponible"))
  ...
  @Test
  void onEventoDonacionVencida_cuandoFeignFalla_noLanzaExcepcion() {
    when(personasService.obtenerIdPersonaAdministradora()).thenReturn(personaAdminId);
    EventoDonacionVencida event =
        new EventoDonacionVencida(
            donacionIndependienteId, donacionOriginalId, "Alimentos vencidos");
    doThrow(new RuntimeException("notificaciones-service no disponible"))
        .when(notificacionesFeignClient)
        .enviarEvento(any());

    assertDoesNotThrow(() -> listener.onEventoDonacionVencida(event));
  }

  @Test
  void onEventoDonacionFallida_cuandoFeignFalla_noLanzaExcepcion() {
    when(personasService.obtenerIdPersonaAdministradora()).thenReturn(personaAdminId);
    ...
    assertDoesNotThrow(() -> listener.onEventoDonacionFallida(event));
  }
}
